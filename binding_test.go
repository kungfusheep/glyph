...
// ActiveBindings caches its derived slice: calling it every frame (the key-help usage)
// re-derives only when the active router changes or a rebind marks it dirty — so it is
// zero-alloc in steady state, the zero-alloc-per-frame tenet KeyHelp depends on.
func TestActiveBindingsCachedZeroAlloc(t *testing.T) {
	app := NewApp()
	app.SetView(VBox(
		On(Key("j", func() {}).Named("scroll-down")),
		On(Key("k", func() {}).Named("scroll-up")),
		Text("hi"),
	))

	first := app.ActiveBindings()
	if len(first) != 2 {
		t.Fatalf("expected 2 named bindings, got %d", len(first))
	}

	// steady state: same router, no rebind → no allocation across many calls.
	allocs := testing.AllocsPerRun(100, func() { _ = app.ActiveBindings() })
	if allocs != 0 {
		t.Fatalf("ActiveBindings must be zero-alloc in steady state, got %.1f allocs/op", allocs)
	}

	// cache hit returns the SAME backing slice (no re-derive).
	cached := app.ActiveBindings()
	if &cached[0] != &first[0] {
		t.Error("a cached ActiveBindings call should return the same backing slice")
	}

	// InvalidateBindings forces the next call to re-derive — a fresh backing slice.
	app.InvalidateBindings()
	fresh := app.ActiveBindings()
	if &fresh[0] == &cached[0] {
		t.Error("after InvalidateBindings ActiveBindings should re-derive a new slice")
	}
	// ...and then it's cached again (zero-alloc steady state restored).
	if a := testing.AllocsPerRun(100, func() { _ = app.ActiveBindings() }); a != 0 {
		t.Errorf("ActiveBindings should re-cache after a re-derive, got %.1f allocs/op", a)
	}
}
