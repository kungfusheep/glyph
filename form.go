...
//	    // handle valid submission, call f.Reset() etc.
//	})(
...
// OnSubmit sets a callback that fires after Enter validates the form.
...
func (f *FormC) bindings() []binding {
	if f.onSubmit != nil {
		cb := f.onSubmit
		enterBinding := binding{pattern: "<Enter>", handler: func() {
			if f.ValidateAll() {
				cb(f)
			}
		}}
